...
export default function Messages({ user, content, type, createdAt, username }) {


    const userId = localStorage.getItem('userId'); 
    const isMe = userId && user === userId.toString();
    ...
    return (
        <div className={`flex ${isMe ? 'justify-end' : 'justify-start'} mb-4`}>
            <div className={`p-3 rounded-lg max-w-sm shadow-lg ${isMe ? 'text-white' : 'bg-purple-600 text-white'}`}>
            {/* <p className="text-xs text-gray-500 mt-1">{username}</p> */}
            {!isMe && <p className="text-xs text-gray-100 font-bold mt-1">{username}</p>}
                {type === 'file' ? (
                    isImage(content) ? (
                        <div className="relative group">
                            <img src={`http://127.0.0.1:8000/storage/${content.split('public/').pop()}`} alt="Uploaded file" className="rounded-lg max-w-xs" />
